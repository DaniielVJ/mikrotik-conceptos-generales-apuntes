## ¿Para que sirve?
El DNS sirve para resolver o relacionar un nombre a una direccion ip. permitiendo que podamos conectarnos a una direccion ip usando un nombre.
...
- Si el mikrotik tiene configurado dhcp clients, este puede utilizar los SV DNS que obtengan de estos procesos dhcp client.
- Mikrotik permite que podamos agregar entradas DNS estaticas, permitiendo agregar nuestros propios nombres e ips a las que debe resolverse si se quiere comunicar a esos nombres.
- La configuracion por defecto trae una entrada estatica de router o router.lan a la dirección ip 192.168.88.1 y esta config por defecto configura a los dispositivos que se conectan al mikrotik como servidor DNS al mismo mikrotik, haciendo que si nos conectamos a router.lan resuelva a la ip del mikrotik 192.168.88.1 y nos podamos comunicar a el usando un nombre.

- Los servidores DNS escuchan peticiones en el puerto 53 UDP. 
- Podemos configurar al mikrotik para que procese peticiones DNS al puerto 53 si van con destino a una direccion ip de las interfaces del mikrotik, y el se encargue de resolver el nombre enviado en la peticion a una dirección ip. Los pasos del proceso son:
    2. Si no esta: revisa en su cache si se encuentra el nombre para resolver. Si esta le envia la ip.
    1. Recibe la peticion, Consulta sus entradas estaticas. Si esta devuelve la ip de la entrada estatica.
...
## Winbox
1. Ver y agregar servidores DNS para que use nuestro Mikrotik.
    - **IP -> DNS** 
...
3. Permitir que el router mikrotik resuelva los nombres a ip que soliciten otros equipos
    - **IP -> DNS**
        - [✅] Allow Remote Request | (Marcar esta opción le dice al mikrotik que si recibe un paquete DNS con ip destino la de una de sus interfaces, el resolvera el nombre a una ip para ese cliente que consulta)
## Notas
...
Tip de Ingeniero: Si quieres bloquear un sitio (ej. facebook.com), puedes crear una entrada estática para ese nombre que apunte a 127.0.0.1. El router la encontrará primero y el cliente nunca llegará al servidor real.
     - (Esto se puede saltar si el dispositivo final configura en su archivo hosts una entrada estatica para facebook.com a su ip real, esto provocara que resuelva a esa ip sin consultar a su server DNS que seria el mikrotiks)
